...
export default {
  ...
  mounted () {
    ...
    var urdfClient = new ROS3D.UrdfClient({
      ros: this.ros,
      tfClient: this.tfClient,
      path: 'http://10.178.253.143/catkin_ws/src',
      rootObject: this.viewer.scene
    })
    ...
    processDomEvent (domEvent) {
      ...
      if (domEvent.type.indexOf('touch') !== -1) {
        ...
        for (var i = 0; i < domEvent.touches.length; ++i) {
          posX += domEvent.touches[i].clientX
          posY += domEvent.touches[i].clientY
        }
        posX /= domEvent.touches.length
        posY /= domEvent.touches.length
      } else {
      ...
      if (domEvent.type === 'mousedown' && domEvent.button === 2) { // Right click
        ...
        } else if (this.screenPosition == null || Math.sqrt((this.screenPosition[0] - posX) * (this.screenPosition[0] - posX) + (this.screenPosition[1] - posY) * (this.screenPosition[1] - posY)) > this.longPressTolerance) {
          this.stopTimer()
        }
