<template>
  <div>
    <el-form ref="loginForm" status-icon :model="form" :rules="rules" label-width="80px" class="login-box">
  ...
  methods: {
    onSubmit (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.userInfo.findIndex(el => el.name === this.form.username) === -1 ||
            this.form.password !== this.userInfo.find(el => el.name === this.form.username).password) {
            this.dialogVisible2 = true
            return false
          } else {
            // 当前登录用户，1小时过期
            this.$cookies.set('user_session', {
              ...this.userInfo.find(el => el.name === this.form.username)}, '360MIN')
